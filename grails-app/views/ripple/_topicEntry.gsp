	...
	<div class="topicText card-body">
		...
		<sec:ifLoggedIn>
			<g:if test="${ripple.responses}">
				<button type="button" class="float-left btn btn-link btn-sm show-responses-button" data-responses-to-button="${ripple.id}">See Replies</button>
			</g:if>
			<button type="button" class="float-right btn btn-link btn-sm reply-button" data-in-reply-to-button="${ripple.id}">Reply</button>
		</sec:ifLoggedIn>
	</div>
